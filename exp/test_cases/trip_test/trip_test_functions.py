...
def get_nml_diag(test_case_name):
    ...
    if 'strat_nudging' in test_case_name:
        sys.path.insert(0, os.path.join(GFDL_BASE, 'exp/test_cases/strat_nudging/'))
        from strat_nudging_test_case import exp as exp_temp
        input_files = exp_temp.inputfiles
        nml_out = exp_temp.namelist
        codebase_to_use = IscaCodeBase
